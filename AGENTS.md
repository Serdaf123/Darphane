...
- **KESİN TASARIM KURALI** (12.09.2026, Serkan): **A ve B yalnız reçeteyle** yapılır (`data/recipes`: tema + bölüm sırası + klasik düzenler); **konsept detayı içermez** (`hours: dial|strip`, `services: fis`, `location: card`, `cta: tabela` A/B'de yasak — test `tests/recipes.test.mts` bunu denetler). **C her zaman bir konsepttir** (`.c.json` = `{"concept": "<key>"}`, uygunluğa göre seçilir). 15 tam tasarım `components/konsept/<key>` (`/konsept` vitrini). Site ya da katman JSON'una `"concept": "tabela"` yazınca sayfa o konseptle çizilir; içerik `sections`'tan (`facts.ts`), teklif şeridi/A-B/analitik aynı kalır. Uygulama: `npm run recipe -- <slug> <konsept> --variant c --apply` (aynı adlı reçete varsa `--concept`). Yeni konsept: bileşen + CSS + `lib/concept-keys.ts` + `registry.ts` + `docs/konseptler.md`.
