 ...
 *
 * Kayıt (session replay) PostHogClient'ta açık; buradakiler kaydın yanına
 * etiket düşer. posthog-js yalnız anahtar varsa ve ilk boyamadan sonra gelir.
 */
  ...
  useEffect(() => {
    ...
    import("posthog-js").then(({ default: posthog }) => {
      const start = () => {
        ...
        stop = track(posthog, slug, status, variant);
      };
      start();
    });
    return () => {
...
function track(posthog: PH, slug: string, status: string, variant: string): () => void {
