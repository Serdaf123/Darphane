...
- **Lighthouse ölçümü (2026-09-12):** bu Mac'te headless Chrome'da ilk boyama sayfadan bağımsız ~2 sn geç gözleniyor (kontrol: `/gizlilik` de aynı); simüle kısıtlamada tek koşu 62–100 arası zıplıyor. Güvenilir ölçüm: `--throttling-method=devtools` (altı sayfa 94–97) ya da CI'daki LHCI / PageSpeed Insights (günlük kota var). Tek simüle koşuya bakıp perf kararı verme; 3 koşunun en iyisi ya da devtools.
...
- **2026-09-12** — 15 konsept (`/konsept`, `components/konsept/`, `docs/konseptler.md`) Serkan'ın kararıyla **arşivde**: "aşırı iyi ama bunları kullanmayalım, kaydedelim". Sitelere bağlanmayacak; ileride fikir/parça kaynağı. Reçeteler de beğenilmedi (kütüphane duruyor, kullanılmıyor). Tasarım yönü kararı bekliyor.
...
- **2026-09-05** Canlı: https://fourpear.vercel.app (Vercel, Serdaf123). robots/noindex/OG canlıda doğrulandı. Kök sayfa giriş ekranı `/giris` (imzalı çerez; parola Vercel env `DARPHANE_ADMIN_PASSWORD`).
...
- **2026-09-07 14:00** CI denetimi: `main`'deki her CI koşusu eklendiği günden beri kırmızıydı (kimse bakmamış). İki neden: (1) `PageProps`/`LayoutProps` Next'in ürettiği global tipler; CI'da typecheck build'den önce koştuğu için `.next/types` yoktu → `typecheck` scripti artık `next typegen && tsc --noEmit`. (2) `.lighthouserc.json`'daki `preset: "mobile"` Lighthouse'ta yok (mobil varsayılan) → kaldırıldı. Yerel LHCI: kök 89/100/96/100, salon-ada 82/100/96/69, olympos 86/100/96/69 (SEO 69 = noindex, bilerek; uyarı düzeyinde kalır). **Ders:** push sonrası GitHub Actions sonucuna bak; `api.github.com/repos/Serdaf123/Darphane/actions/runs` anahtarsız okunur. **Açık konu:** repo GitHub'da **public** görünüyor (05.09 kararı private idi); içinde teklif metinleri, fiyatlar, işletme telefonları var. Serkan Settings → Danger zone'dan private'a çevirmeli (Vercel ve Codex Serdaf123 hesabıyla bağlı, etkilenmez).
- **2026-09-08** Serkan'ın isteğiyle site temizliği: Noyavet ve kişisel site dışındaki adaylar silindi (Olympos +EN, avukat, Yellow Bull A/B, Salon Ada, av-varyant b/c/d, pitch metinleri, shots). Demolar (Ocakbaşı, Diş) kişisel sitenin "Örnek çalışmalar" bölümü onlara link verdiği için Serkan'ın kararıyla kaldı. Lighthouse CI artık `/`, `/ocakbasi-sahin`, `/noyavet`; `draft-site` örneği noyavet.json. MANUS_API_KEY girildi ve doğrulandı; ilk görev (brif 01) Manus'ta **kredi yetersiz** hatasıyla düştü (görev d558XZCNWPL4RWRiFXAuiR) → kredi yüklenince tekrar.
- **2026-09-08** Manus: `standard` profil günlük (ücretsiz) krediyle "quota_limit" veriyor, `--profile lite` çalışıyor (52 sn, 3 KB). Bundan sonra varsayılan lite. Brif 01 çıktısı `content/serkan/copy.md`: olgu ihlali yok, ünlem/pazarlama dili yok; zayıf yanlar: biz/ben karışık (açılış "hazırlıyoruz", süreç "hazırlıyorum"), "beğenmezse kaldırılır" ve İngilizce sürüm olguları geçmiyor, alan adı sahipliği SSS'si kaçamak. Tasarım A'ya entegrasyon Serkan'ın kararıyla.
